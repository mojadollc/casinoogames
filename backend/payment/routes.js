...
router.post('/deposit', authenticate, async (req, res) => {
  ...
  try {
    ...
    await query(
      ...
      [req.user.id, 'deposit', amount, invoice.data.id, 'pending', JSON.stringify({ invoice_url: invoice.data.invoice_url })]
    );
    await query('INSERT INTO deposit_requests (id, user_id, amount, payment_method, provider_ref) VALUES (UUID(),?,?,?,?)',
      [req.user.id, amount, payment_method || null, invoice.data.id]);
...
router.post('/withdraw', authenticate, async (req, res) => {
  ...
  try {
    ...
    await debitWallet(req.user.id, amount, 'withdrawal', 'Withdrawal request - pending approval', null);

    const inserted = await query(
      'INSERT INTO withdrawal_requests (id, user_id, amount, bank_code, account_number, account_name, status) VALUES (UUID(),?,?,?,?,?,\'pending\')',
      [req.user.id, amount, bank_code, account_number, account_name]
    );
    ...
    try {
      ...
      if (io) io.to('admins').emit('withdrawal:new', { id: wr.rows[0]?.id, amount, username: req.user.username });
    } catch {}
...
const webhookHandler = async (req, res) => {
  ...
  try {
    if (external_id?.startsWith('dep_')) {
      if (status === 'PAID') {
        const tx = await query('SELECT * FROM payment_transactions WHERE provider_ref = ?', [id]);
        if (tx.rows[0] && tx.rows[0].status === 'pending') {
          await creditWallet(tx.rows[0].user_id, amount, 'deposit', 'Xendit deposit', id);
          await updateAffiliationOnDeposit(tx.rows[0].user_id, amount);
          await query('UPDATE payment_transactions SET status = ?, webhook_data = ?, updated_at = NOW() WHERE provider_ref = ?',
            ['completed', JSON.stringify(req.body), id]);
          await query('UPDATE deposit_requests SET status = ? WHERE provider_ref = ?', ['completed', id]);
        }
      ...
      if (status === 'FAILED') {
        // Look up user_id from the withdrawal request — Xendit does not send it
        const wd = await query('SELECT user_id FROM withdrawal_requests WHERE id = ?', [wdId]);
        if (wd.rows[0]) {
          await creditWallet(wd.rows[0].user_id, amount, 'refund', 'Payout failed - refund', wdId);
        }
      }
    }
