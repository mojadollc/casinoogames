...
router.post('/wallets/adjust', adminAuth, async (req, res) => {
  ...
  if (amount > 0) await creditWallet(userId, amount, 'admin_adjustment', reason, req.user.id);
  else {
    const { debitWallet } = require('../wallet/routes');
    await debitWallet(userId, Math.abs(amount), 'admin_adjustment', reason, req.user.id);
  }
  await query('INSERT INTO admin_logs (id, admin_id, action, target_type, target_id, details) VALUES (UUID(), ?, ?, ?, ?, ?)',
    [req.user.id, 'wallet_adjustment', 'wallet', userId, JSON.stringify({ amount, reason })]);
...
router.post('/withdrawals/:id/approve', adminAuth, async (req, res) => {
  try {
    ...
    await query('UPDATE withdrawal_requests SET approved_by = ?, status = \'approved\', updated_at = NOW() WHERE id = ?',
      [req.user.id, req.params.id]);
    await query('UPDATE wallet_transactions SET description = \'Withdrawal approved\' WHERE reference_id = ? AND type = \'withdrawal\'',
      [req.params.id]);
    // Notify user
    try {
