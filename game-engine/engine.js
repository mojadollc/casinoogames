...
class GameEngine {
  ...
  getRandomSymbol(forceWin = false) {
    ...
    // If forcing win, bias towards high-value symbols
    if (forceWin) {
      const winSymbols = symbols.filter(s => s.payout[3] >= 10);
  ...
  spin(betAmount, isFreeSpin = false, sessionStats = { totalBet: 0, totalWin: 0, spins: 0 }) {
    ...
    else if (this.settings.forceOutcome === 'win') {
      const result = this.generateWinningGrid(betAmount);
      grid = result.grid;
      forcedOutcome = 'win_forced';
    }
  ...
  static createForcedOutcome(type, betAmount) {
    ...
    return engine.spin(betAmount, false);
  }
}
