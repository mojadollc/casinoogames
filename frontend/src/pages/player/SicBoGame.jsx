...
export default function SicBoGame() {
  ...
  const roll = async () => {
    ...
    try {
      // Use backend spin to determine win/loss (respects win_rate / force_outcome)
      const { data } = await gameAPI.spin(game.id, totalBet);
      ...
      if (won) {
        // Generate dice that match a winning bet
        const betTypes = Object.keys(bets);
        const winBet = betTypes[Math.floor(Math.random() * betTypes.length)];
        finalDice = getDiceForBet(winBet);
      } else {
        // Generate dice that don't match any placed bet
        finalDice = getDiceAvoidingBets(bets);
      }
      ...
      if (won) {
        // Calculate winnings based on actual dice result
        if (bets.big && total >= 11 && total <= 17) winnings += bets.big * 2;
        if (bets.small && total >= 4 && total <= 10) winnings += bets.small * 2;
        if (bets.odd && total % 2 === 1) winnings += bets.odd * 2;
        if (bets.even && total % 2 === 0) winnings += bets.even * 2;
        if (bets[`total_${total}`]) {
          const multipliers = { 4: 12, 5: 8, 6: 6, 7: 5, 8: 4, 9: 3, 10: 3, 11: 3, 12: 4, 13: 5, 14: 6, 15: 8, 16: 12, 17: 12 };
          winnings += bets[`total_${total}`] * (multipliers[total] || 3);
        }
        const hasDouble = finalDice.some((v, i) => finalDice.indexOf(v) !== i);
        if (bets.triple && finalDice[0] === finalDice[1] && finalDice[1] === finalDice[2]) winnings += bets.triple * 30;
        if (bets.double && hasDouble) winnings += bets.double * 8;
        // Fallback: if dice didn't match any bet, use backend win amount
        if (winnings === 0) winnings = data.totalWin;
      }
