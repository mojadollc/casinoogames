...
export default function LiveGame() {
  ...
  const spin = async () => {
    ...
    try {
      ...
      // Pick a segment based on backend outcome
      const won = data.totalWin > 0;
      let segment;
      if (won) {
        // Pick a winning segment (multiplier > 1 or any segment if all are 1)
        const winSegs = config.segments.filter(s => s.multiplier > 1);
        segment = winSegs.length > 0
          ? winSegs[Math.floor(Math.random() * winSegs.length)]
          : config.segments[Math.floor(Math.random() * config.segments.length)];
      } else {
        // Pick lowest multiplier segment
        const sorted = [...config.segments].sort((a, b) => a.multiplier - b.multiplier);
        segment = sorted[0];
      }
      ...
      setHistory(prev => [{ segment, win: data.totalWin }, ...prev].slice(0, 8));
