...
// Symbol definitions — values match engine DEFAULT_CONFIG (5-of-a-kind multiplier)
const SYMBOLS = {
  wild:       { icon: '🐉', color: '#ffd700', value: 100, name: 'Wild Dragon' },
  scatter:    { icon: '💎', color: '#00f5d4', value: 25,  name: 'Scatter' },
  seven:      { icon: '7️⃣', color: '#ff2d75', value: 75,  name: 'Lucky Seven' },
  crown:      { icon: '👑', color: '#ffd700', value: 40,  name: 'Crown' },
  ...
  watermelon: { icon: '🍉', color: '#2ecc71', value: 7,   name: 'Watermelon' }
};
...
export default function SlotGame() {
  ...
  const spin = async () => {
    ...
    try {
      ...
      await spinReels(data.grid || generateRandomReels());
  ...
  const useFreeSpin = async () => {
    ...
    try {
      ...
      setBalance(data.balance);
      setLastWin(data.totalWin || 0);
      setFreeSpins(data.freeSpinsRemaining || 0);
